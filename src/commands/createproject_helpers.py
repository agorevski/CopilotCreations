"""
Helper functions for project creation commands.

This module contains extracted helper functions from createproject.py
to enable reuse across createproject and session_commands.
"""
...
def _generate_summary_section(
    ...
    if not is_complete:
        ...
        status = (
            f"⏰ **TIMED OUT** - Process was killed after {TIMEOUT_MINUTES} minutes"
        )
    elif error_occurred:
        status = format_error_message(
...
def _build_unified_message(
    folder_section: str, output_section: str, summary_section: str
) -> str:
...
async def update_unified_message(
    ...
    while is_running.is_set() and not error_event.is_set():
        try:
            ...
            content = _build_unified_message(
            ...
            if content != last_content:
                try:
                    ...
                    if e.code == DISCORD_INVALID_WEBHOOK_TOKEN:
                        logger.info(
                            "Interaction token expired during updates, re-fetching message"
                        )
...
async def create_project_directory(
    username: str, session_log: SessionLogCollector, prompt: str = ""
) -> Tuple[Path, str]:
...
"""
    ...
    session_log.info("Created COPILOT-PROMPT.md with original prompt")
...
async def run_copilot_process(
    ...
    try:
        ...
        try:
            ...
            session_log.info(
                f"Copilot process completed with exit code: {process.returncode}"
            )
...
async def update_final_message(
    ...
    try:
        ...
        is_success = (
            not timed_out and not error_occurred and process and process.returncode == 0
        )
        ...
        if is_success:
            ...
            display_description = (
                description if description else "(No description generated)"
            )
...
async def handle_github_integration(
    ...
    logger.info(
        f"GitHub check: enabled={GITHUB_ENABLED}, timed_out={timed_out}, "
        f"error_occurred={error_occurred}, returncode={process.returncode if process else 'None'}"
    )
    ...
    if (
        ...
        if github_manager.is_configured():
            ...
            if naming_generator.is_configured():
                ...
                if repo_description:
                    session_log.info(
                        f"Generated repository description: {repo_description}"
                    )
            ...
            logger.warning(
                "GitHub enabled but not configured (missing GITHUB_TOKEN or GITHUB_USERNAME)"
            )
            github_status = "\n**🐙 GitHub:** Not configured (set GITHUB_TOKEN and GITHUB_USERNAME in .env)"
    elif GITHUB_ENABLED and (
        timed_out or error_occurred or (process and process.returncode != 0)
    ):
...
def cleanup_project_directory(
    project_path: Path, session_log: SessionLogCollector
) -> bool:
...
async def _send_log_file(
    ...
    try:
        log_file = discord.File(
            io.BytesIO(log_markdown.encode("utf-8")), filename=f"{folder_name}_log.md"
        )
